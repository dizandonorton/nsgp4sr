function ind=MASE(ind,params,data,terminals,varsvals)
%MASE (Mean Absolute Scaled Error) is a measure of the accuracy of
%forecasts
...
n = size(res,1);
numerator = sum(abs(data.result-res));
denominator = 0.0;
for i=2:n
	for j=i-1:n
		denominator = denominator + abs(data.result(i)-data.result(j));
	end
end
result = ((n-1)*numerator)/(n*denominator);
%%

ind.result=res;

% raw fitness:
ind.fitness=result; %lower fitness means better individual
