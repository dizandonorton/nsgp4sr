function [individual,state]=calcfitness(individual,params,data,state,testdata,pop)
...
if isempty(state.keepevals) % this means keepevals has not be initialized (on purpose)
   % IF FITNESS FUNCTION IS NOVELTY SEARCH ADD ALSO THE POPULATION
   if strcmp(params.calcfitness,'noveltysearch')
      %individual=feval(params.calcfitness,individual,params,data,state.terminals,pop,state.initpopsize,v);
      individual=feval(params.calcfitness,individual,params,data,state,v);
   else
