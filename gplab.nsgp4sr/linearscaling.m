function ind=linearscaling(ind,params,data,terminals,varsvals)
...
mean_t = mean(data.result);
mean_y = mean(res);
%b = cov(data.result, res)/var(res);
b = sum((data.result - mean_t)'*(res - mean_y))/sum((res - mean_y).^2);

%calculating the interception 'a' and 'a+by'
a = mean_t - b*mean_y;
aplusby = a + b*res;

%calculates the rmse on: a + by
sumdif=sqrt(mean((aplusby-data.result).^2));
%%%% END OF MY CODE
